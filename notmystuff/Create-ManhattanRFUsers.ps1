$wmsforkliftpw = (ConvertTo-SecureString "Cinco123!" -AsPlainText -force)
...
foreach ($user in $users) 
    {
        try {
            New-ADUser -SamAccountName $user.SamAccountName -Name ($user.FirstName + " " + $user.LastName + ".wms") `
            -Path ($user.Path) `
            -DisplayName ($user.FirstName + " " + $user.LastName + ".wms") -GivenName $user.FirstName -Surname $user.LastName `
            -EmailAddress ($user.SamAccountName + ".wms" + $dnsroot) -UserPrincipalName ($user.SamAccountName + ".wms" + $dnsroot) `
            -Description $user.Description `
            -Enabled $true -ChangePasswordAtLogon $false -PasswordNeverExpires  $true -CannotChangePassword $true `
            -AccountPassword $wmsforkliftpw -PassThru `
            }
